	...
	*region = NULL;
	...
	list_for_each_entry(region_elt, &audio->ion_region_queue, list) {
		if (addr >= region_elt->vaddr &&
			...
			addr + len <= region_elt->vaddr + region_elt->len && addr + len > addr) {
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 16-048 */
	if (ref_up)
		region->ref_cnt = 1;
	else
		region->ref_cnt = 0;
#else  /* CONFIG_SH_AUDIO_DRIVER */ /* 16-048*/
	if (ref_up)
		region->ref_cnt++;
	else
		region->ref_cnt--;
#endif  /* CONFIG_SH_AUDIO_DRIVER */ /* 16-048 */
