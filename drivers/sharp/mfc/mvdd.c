...
{
	if(pull_type == D_INTU_PULL_UP){
		gpio_tlmm_config(GPIO_CFG(D_INTU_GPIO_NO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP  , GPIO_CFG_2MA), 1);
	}else{
		gpio_tlmm_config(GPIO_CFG(D_INTU_GPIO_NO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), 1);
	}
}
#endif /* CONFIG_SHSNFC */
...
static void snfc_output_disable(void)
{
	...
	/* UART-TX */
	ret = gpio_tlmm_config(GPIO_CFG(D_UART_TX_GPIO_NO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if(ret) {
		MFC_DRV_ERR_LOG("gpio_tlmm_config ret = %d", ret);
	}

	/* UART-RX */
	ret = gpio_tlmm_config(GPIO_CFG(D_UART_RX_GPIO_NO, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if(ret) {
		MFC_DRV_ERR_LOG("gpio_tlmm_config ret = %d", ret);
	}
