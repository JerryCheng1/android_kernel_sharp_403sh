...
#define D_UART_CTS_GPIO_NO		(6)
#define D_UART_RTS_GPIO_NO		(7)
#define D_VFEL_GPIO_NO			g_vfel_gpio_no

#define D_PVDD_GPIO_NO			(62)
#define D_TVDD_GPIO_NO			(112)
#define D_AVDD_GPIO_NO			(50)
...
static void snfc_chip_reset(void)
{
	...
	ret = gpio_request(D_VFEL_GPIO_NO, "nfc_vfel");
	if (ret) {
		NFC_DRV_ERR_LOG("failed to gpio_request vfel");
	}
...
static int snfc_pvdd_vreg_enable(int enable)
{
	...
	if(enable){
		gpio_set_value(D_PVDD_GPIO_NO, 1);
	} else {
		gpio_set_value(D_PVDD_GPIO_NO, 0);
	}
...
static int snfc_avdd_vreg_enable(int enable)
{
	...
	if(enable){
		gpio_set_value(D_AVDD_GPIO_NO, 1);
	} else {
		gpio_set_value(D_AVDD_GPIO_NO, 0);
	}
...
static int snfc_tvdd_vreg_enable(int enable)
{
	...
	if(enable){
		gpio_set_value(D_TVDD_GPIO_NO, 1);
	} else {
		gpio_set_value(D_TVDD_GPIO_NO, 0);
	}
...
static __init int snfc_en_init(void)
{
	...
	g_ven_gpio_no  = qpnp_pin_map("pm8226-gpio", 3);
	g_vfel_gpio_no = qpnp_pin_map("pm8226-gpio", 6);
