...
/* msm-gpio */
#define SHBATT_GPIO_TPIN_STATUS		49
...
static int64_t lcd_fixed_temp = 25;
...
static shbatt_result_t shbatt_control_read_adc_channel( shbatt_adc_t* adc_p, uint32_t* microvolts )
{
	...
	if(adc_p->channel == SHBATT_ADC_CHANNEL_DCIN ||
...
#ifdef CONFIG_PM_LCD_THERM
		if(adc_p->channel == SHBATT_ADC_CHANNEL_LCD_THERM)
		{
			rev = sh_get_hw_revision();
			rev = rev & 0x07;
			SHBATT_TRACE("[%s] rev:%x \n",__FUNCTION__, rev);
			if((rev == HW_ES0) || rev == HW_ES1)
			{
				SHBATT_TRACE("[%s] use fixed temperature\n", __FUNCTION__);
				adc_p->physical = lcd_fixed_temp;
			}
		}
#endif /* CONFIG_PM_LCD_THERM */
...
static shbatt_result_t shbatt_control_get_temperature( shbatt_adc_channel_t channel, int* tmp_p )
{
    ...
    SHBATT_TRACE("[P]%s() channel:%d", channel);
    adc.channel = channel;
...
static ssize_t shbatt_drv_show_battery_property( struct device* dev_p, struct device_attribute* attr_p ,char* buf_p )
{
	...
	case SHBATT_PS_PROPERTY_STATUS:
#ifndef CONFIG_MOBILE_LIGHT_R46683
		intval = shbatt_ps_battery_get_prop_status();
#else
		if(qpnp_chg_torch_is_enable() != 0)
			intval = SHBATT_CHG_STATUS_DISCHARGING;
		else
			intval = shbatt_ps_battery_get_prop_status();
#endif
		...
		if(0)
		{
			/* remove this function, since it is unnecessary and surplus. */
