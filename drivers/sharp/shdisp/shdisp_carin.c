...
#define SHDISP_CARIN_ALPHA_DEFAULT           0x0132
...
static int shdisp_carin_API_diag_write_reg(unsigned char addr, unsigned char *write_data, unsigned char size, int power_mode);
static int shdisp_carin_API_diag_read_reg(unsigned char addr, unsigned char *read_data, unsigned char size, int power_mode);
...
static int shdisp_carin_API_set_irq(int enable);
...
static int shdisp_carin_init_flicker_param(unsigned short vcom);
static int shdisp_carin_diag_set_flicker_param_internal(struct shdisp_diag_flicker_param flicker_param);
static int shdisp_carin_diag_set_flicker_param_ctx(struct shdisp_diag_flicker_param flicker_param);
...
/* MACROS                                                                    */
/* ------------------------------------------------------------------------- */
#define MIPI_DSI_COMMAND_TX(x)         (shdisp_panel_API_mipi_dsi_cmds_tx(0,x, ARRAY_SIZE(x),0x00))
#define MIPI_DSI_COMMAND_TX_COMMIT(x,flags)  (shdisp_panel_API_mipi_dsi_cmds_tx(1,x, ARRAY_SIZE(x),flags))
...
static int shdisp_carin_API_init_io(void)
{
    ...
    shdisp_carin_init_flicker_param(shdisp_panel_ctx.vcom);
...
static int shdisp_carin_API_diag_write_reg(unsigned char addr, unsigned char *write_data, unsigned char size, int power_mode)
{
    ...
    ret = shdisp_panel_API_mipi_diag_write_reg(dtype,addr, write_data, size, power_mode);
    if(ret) {
        SHDISP_DEBUG("out1\n");
        return SHDISP_RESULT_FAILURE;
    }
...
static int shdisp_carin_API_diag_read_reg(unsigned char addr, unsigned char *read_data, unsigned char size, int power_mode)
{
    ...
    ret = shdisp_panel_API_mipi_diag_read_reg(dtype,addr, read_data,size, power_mode);
...
static int shdisp_carin_API_diag_set_flicker_param(struct shdisp_diag_flicker_param flicker_param)
{
    ...
    ret = shdisp_carin_diag_set_flicker_param_internal(flicker_param);

    if (ret != SHDISP_RESULT_SUCCESS) {
        SHDISP_ERR("<RESULT_FAILURE> shdisp_carin_diag_set_flicker_param_internal.\n");
        return SHDISP_RESULT_FAILURE;
    } else {
        shdisp_carin_diag_set_flicker_param_ctx(flicker_param);
    }
...
static int shdisp_carin_API_diag_get_flicker_param(struct shdisp_diag_flicker_param *flicker_param)
{
    ...
    ret = shdisp_panel_API_mipi_diag_read_reg(SHDISP_DTYPE_GEN_READ2,0xD5, carin_rdata, 7,SHDISP_CMD_REQ_LP_MODE);
    if(ret) {
...
static int shdisp_carin_API_diag_set_gamma_info(struct shdisp_diag_gamma_info *gamma_info)
{
    ...
    ret = shdisp_panel_API_mipi_diag_write_reg(SHDISP_DTYPE_GEN_LWRITE,SHDISP_CARIN_GAMMA_SETTING,
                                               gamma_info->gamma,
                                               SHDISP_CARIN_GAMMA_SETTING_SIZE,SHDISP_CMD_REQ_LP_MODE);
    if(ret) {
    ...
    ret = shdisp_panel_API_mipi_diag_read_reg(SHDISP_DTYPE_GEN_READ2,SHDISP_CARIN_POWER_SETTING,
                                              carin_rdata,
                                              SHDISP_CARIN_POWER_SETTING_SIZE,SHDISP_CMD_REQ_LP_MODE);
    if(ret) {
        SHDISP_ERR("<RESULT_FAILURE> mipi_sharp_diag_read_reg.\n");
        return SHDISP_RESULT_FAILURE;
    }
    ...
    ret = shdisp_panel_API_mipi_diag_read_reg(SHDISP_DTYPE_GEN_READ2,SHDISP_CARIN_POWER_SETTING_FOR_INTERNAL_POWER,
                                              carin_rdata,
                                              SHDISP_CARIN_POWER_SETTING_FOR_INTERNAL_POWER_SIZE,SHDISP_CMD_REQ_LP_MODE);
    if(ret) {
        SHDISP_ERR("<RESULT_FAILURE> mipi_sharp_diag_read_reg.\n");
        return SHDISP_RESULT_FAILURE;
    }
    ...
    chargePumpSetting_payloads[SHDISP_CARIN_VLM1] = gamma_info->vlm1;
    chargePumpSetting_payloads[SHDISP_CARIN_VLM2] = gamma_info->vlm2;
    chargePumpSetting_payloads[SHDISP_CARIN_VLM3] = gamma_info->vlm3;
    internalPower_payloads[SHDISP_CARIN_VC1] = gamma_info->vc1;
    internalPower_payloads[SHDISP_CARIN_VC2] = gamma_info->vc2;
    internalPower_payloads[SHDISP_CARIN_VC3] = gamma_info->vc3;
    internalPower_payloads[SHDISP_CARIN_VPL] = gamma_info->vpl;
    internalPower_payloads[SHDISP_CARIN_VNL] = gamma_info->vnl;
    internalPower_payloads[SHDISP_CARIN_SVSS_SVDD] = gamma_info->svss_svdd;
    memcpy(&gammmaSettingCommonSet_payloads[1],gamma_info->gamma,SHDISP_CARIN_GAMMA_SETTING_SIZE);
...
#define PANEL_DUMP_RECORD(a) {a, sizeof(a)/sizeof(*a)}
    ...
    for(reccnt = 0; reccnt < reclen; reccnt++) {
        ...
        for(cmdcnt = 0; cmdcnt < dumpary[reccnt].len; ++cmdcnt, ++cmdpos){
            ...
            res = shdisp_panel_API_mipi_dsi_cmds_rx(readbuf, &readcmd, cmdpos->dlen-1,SHDISP_CMD_REQ_LP_MODE);
...
static int shdisp_carin_API_set_irq(int enable)
{
	...
	ret = shdisp_carin_set_irq(enable);
...
static char shdisp_carin_mipi_manufacture_id(void)
{
    ...
    res = shdisp_panel_API_mipi_dsi_cmds_rx(rbuf, cmd, sizeof(rbuf),SHDISP_CMD_REQ_LP_MODE);
    if (res == SHDISP_RESULT_SUCCESS) {
...
static int shdisp_carin_mipi_cmd_start_display(void)
{
    ...
    ret = MIPI_DSI_COMMAND_TX_COMMIT(disp_on_exit_sleep_cmds,SHDISP_CMD_REQ_HS_MODE);
    SHDISP_TRACE("out ret=%d\n", ret);
    return ret;
}
...
static int shdisp_carin_mipi_cmd_lcd_off(void)
{
    ...
    ret = MIPI_DSI_COMMAND_TX_COMMIT(deep_standby_cmds,SHDISP_CMD_REQ_HS_MODE);
    if (ret != SHDISP_RESULT_SUCCESS) {
...
static int shdisp_carin_mipi_cmd_lcd_on(void)
{
    ...
    ret = MIPI_DSI_COMMAND_TX_COMMIT(initial_setting_1_1_cmds,SHDISP_CMD_REQ_HS_MODE);
    if (ret != SHDISP_RESULT_SUCCESS){
    ...
    ret = MIPI_DSI_COMMAND_TX_COMMIT(sync_signal_setting_cmds,SHDISP_CMD_REQ_HS_MODE);
...
#ifndef SHDISP_NOT_SUPPORT_FLICKER
/* ------------------------------------------------------------------------- */
/* shdisp_carin_init_flicker_param                                           */
/* ------------------------------------------------------------------------- */
static int shdisp_carin_init_flicker_param(unsigned short vcom)
{
    SHDISP_TRACE("in\n");
    ...
    power_setting_payloads[4] = ((vcom >> 8) & 0x01);
    power_setting_payloads[5] = (vcom & 0xFF);
    power_setting_payloads[6] = ((vcom >> 8) & 0x01);
    power_setting_payloads[7] = (vcom & 0xFF);
...
/* ------------------------------------------------------------------------- */
/* shdisp_carin_diag_set_flicker_param_internal                              */
/* ------------------------------------------------------------------------- */
static int shdisp_carin_diag_set_flicker_param_internal(struct shdisp_diag_flicker_param flicker_param)
{
    int i;
    int ret = 0;
    unsigned short vcom = flicker_param.master_alpha;
    ...
    if (flicker_param.request & SHDISP_REG_WRITE) {
        ...
        ret = shdisp_panel_API_mipi_diag_write_reg(SHDISP_DTYPE_GEN_LWRITE,0xD5, carin_wdata, 7,SHDISP_CMD_REQ_LP_MODE);
        if(ret) {
            SHDISP_ERR("<RESULT_FAILURE> mipi_sharp_diag_write_reg.\n");
            return SHDISP_RESULT_FAILURE;
        }
    ...
    if (flicker_param.request & (SHDISP_SAVE_VALUE | SHDISP_SAVE_VALUE_LOW)) {
        ...
        if (!(flicker_param.request & SHDISP_SAVE_VALUE_LOW)) {
            SHDISP_WARN("carin don't have param for low drive VCOM.");
        }
        if (shdisp_carin_init_flicker_param(vcom)) {
            SHDISP_ERR("<RESULT_FAILURE> shdisp_carin_init_flicker_param.");
        }
    }

    if (flicker_param.request & SHDISP_RESET_VALUE) {
        if (shdisp_carin_init_flicker_param(flicker_param.master_alpha)) {
            SHDISP_ERR("<RESULT_FAILURE> shdisp_carin_init_flicker_param.");
        }
    }
...
/* ------------------------------------------------------------------------- */
/* shdisp_carin_diag_set_flicker_param_ctx                                   */
/* ------------------------------------------------------------------------- */
static int shdisp_carin_diag_set_flicker_param_ctx(struct shdisp_diag_flicker_param flicker_param)
{
    SHDISP_TRACE("in");
...
int shdisp_panel_carin_API_TestImageGen(int onoff) /* Test Image Generator ON */
{
    ...
    SHDISP_TRACE("in\n");
    if (onoff) {
        ret = MIPI_DSI_COMMAND_TX_COMMIT(TIG_on_cmds,SHDISP_CMD_REQ_LP_MODE);
    }
    else {
        ret = MIPI_DSI_COMMAND_TX_COMMIT(TIG_off_cmds,SHDISP_CMD_REQ_LP_MODE);
    }
    SHDISP_TRACE("out ret=%d\n", ret);
    return ret;
}
...
static int shdisp_carin_mipi_checksum_and_ecc_error_count(char *out)
{
    ...
    ret = shdisp_panel_API_mipi_dsi_cmds_rx(rbuf, cmd, sizeof(rbuf),SHDISP_CMD_REQ_LP_MODE);
    if (ret == SHDISP_RESULT_SUCCESS) {
