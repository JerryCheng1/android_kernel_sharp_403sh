...
static int shdisp_SQE_panel_write_reg(struct shdisp_lcddr_reg *panel_reg, int power_mode);
static int shdisp_SQE_panel_read_reg(struct shdisp_lcddr_reg *panel_reg, int power_mode);
...
/* ------------------------------------------------------------------------- */
/* shdisp_api_panel_digital_power_on                                         */
/* ------------------------------------------------------------------------- */

void shdisp_api_digital_power_on(void)
{
...
static int shdisp_ioctl_set_host_gpio(void __user *argp)
{
    ...
    ret = copy_from_user(&host_gpio,argp,sizeof(struct shdisp_host_gpio));
...
static int shdisp_ioctl_lcddr_write_reg(void __user *argp)
{
    ...
    ret = shdisp_SQE_panel_write_reg(&panel_reg, SHDISP_CMD_REQ_LP_MODE);
...
static int shdisp_ioctl_lcddr_read_reg(void __user *argp)
{
    ...
    ret = shdisp_SQE_panel_read_reg(&panel_reg, SHDISP_CMD_REQ_LP_MODE);
...
static int shdisp_SQE_tri_led_set_color(struct shdisp_tri_led *tri_led)
{
    ...
    if (tri_led->led_mode == SHDISP_TRI_LED_MODE_NORMAL) {
        if ((shdisp_kerl_ctx.tri_led.red      == led.red) &&
            ...
            return SHDISP_RESULT_SUCCESS;
             ...
             (tri_led->led_mode == SHDISP_TRI_LED_MODE_EMOPATTERN)){
        if ((shdisp_kerl_ctx.tri_led.red      == led.red) &&
            (shdisp_kerl_ctx.tri_led.green    == led.green) &&
            (shdisp_kerl_ctx.tri_led.blue     == led.blue) &&
            (shdisp_kerl_ctx.tri_led.ext_mode == tri_led->ext_mode) &&
    ...
    if (xstb_ch012 != 0) {
        ...
        case SHDISP_TRI_LED_MODE_EMOPATTERN:
            shdisp_bdic_API_TRI_LED_emopattern_on( color, tri_led->ontime, tri_led->interval, tri_led->count );
            break;
        default:
...
static int shdisp_SQE_panel_write_reg(struct shdisp_lcddr_reg *panel_reg, int power_mode)
{
    ...
    ret = shdisp_panel_API_diag_write_reg(panel_reg->address, panel_reg->buf, panel_reg->size, power_mode);
...
static int shdisp_SQE_panel_read_reg(struct shdisp_lcddr_reg *panel_reg, int power_mode)
{
    ...
    ret = shdisp_panel_API_diag_read_reg(panel_reg->address, panel_reg->buf, panel_reg->size, power_mode);
...
#if defined(CONFIG_SHDISP_PANEL_MONTBLANC)  ||defined(CONFIG_SHDISP_PANEL_CARIN)
    shdisp_SQE_main_lcd_disp_off();
...
#ifdef SHDISP_ALS_INT
                }else if( nFirstBDIC_GFAC == SHDISP_BDIC_IRQ_TYPE_ALS_TRIGGER ){
                    shdisp_do_als_int_report(SHDISP_OPT_CHANGE_INT_1 | SHDISP_OPT_CHANGE_INT_2);
                }else if( nFirstBDIC_GFAC == SHDISP_BDIC_IRQ_TYPE_ALS_TRIGGER1 ){
                    shdisp_do_als_int_report(SHDISP_OPT_CHANGE_INT_1);
                }else if( nFirstBDIC_GFAC == SHDISP_BDIC_IRQ_TYPE_ALS_TRIGGER2 ){
...
static int shdisp_set_als_int_subscribe(int trigger)
{
    ...
    ret = shdisp_api_event_subscribe(&alsint_subs);
...
static int shdisp_set_als_int_unsubscribe(int trigger)
{
    ...
    als_int_enable = (als_int_enable & (~trigger)) & (SHDISP_OPT_CHANGE_INT_1 | SHDISP_OPT_CHANGE_INT_2);
    if (als_int_enable == 0) {
        ret = shdisp_api_event_unsubscribe(SHDISP_IRQ_TYPE_ALS);
    } else {
        SHDISP_DEBUG("<caution>double unsubscribe");
        return SHDISP_RESULT_SUCCESS;
    }
...
#define SHDISP_PARAM_MAX (4)
    ...
    case SHDISP_DEBUG_PSALS_REG_WRITE:
        printk("[SHDISP] PSALS-REG WRITE (addr : 0x%02x, data : 0x%02x)\n", shdisp_pfs.par[0], shdisp_pfs.par[1]);
    ...
    case SHDISP_DEBUG_PSALS_REG_READ:
        printk("[SHDISP] PSALS-REG READ (addr : 0x%02x)\n", shdisp_pfs.par[0]);
        i2c_wbuf[0] = shdisp_pfs.par[0];
        i2c_rbuf[0] = 0x00;
    ...
    case SHDISP_DEBUG_DSI_WRITE:
        ...
        SHDISP_DEBUG(" PowerMode : %04x\n", power_mode);
        for (i = 0; i < panel_reg.size; i++){
            panel_reg.buf[i] = param[i+3];
            if ((i % 8) == 0) {
        ...
        printk("\n");
        shdisp_SQE_panel_write_reg(&panel_reg, power_mode);
    ...
    case SHDISP_DEBUG_DSI_READ:
        ...
        memset(panel_reg.buf, 0, sizeof(panel_reg.buf));
        panel_reg.cog = SHDISP_DIAG_COG_ID_NONE;
        power_mode = shdisp_pfs.par[1];
        shdisp_SQE_panel_read_reg(&panel_reg, power_mode);
