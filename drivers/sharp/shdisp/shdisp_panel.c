...
#if defined(CONFIG_SHDISP_PANEL_MONTBLANC)
#include "shdisp_montblanc.h"
...
extern int mdss_shdisp_host_dsi_tx(int commit, struct shdisp_dsi_cmd_desc * shdisp_cmds, int size,int flags);
extern int mdss_shdisp_host_dsi_rx(struct shdisp_dsi_cmd_desc * cmds, unsigned char * rx_data, int rx_size,int flags);
...
#if defined(CONFIG_SHDISP_PANEL_MONTBLANC)
    shdisp_panel_fops = shdisp_montblanc_API_create();
#elif defined(CONFIG_SHDISP_PANEL_CARIN)
...
int shdisp_panel_API_diag_write_reg(unsigned char addr, unsigned char *write_data, unsigned char size, int power_mode)
{
    if (shdisp_panel_fops->write_reg) {
        return shdisp_panel_fops->write_reg(addr, write_data, size, power_mode);
    }
    return SHDISP_RESULT_SUCCESS;
}
...
int shdisp_panel_API_diag_read_reg(unsigned char addr, unsigned char *read_data, unsigned char size, int power_mode)
{
    if (shdisp_panel_fops->read_reg) {
        return shdisp_panel_fops->read_reg(addr, read_data, size, power_mode);
    }
    return SHDISP_RESULT_SUCCESS;
}
...
/* ------------------------------------------------------------------------- */
int shdisp_panel_API_mipi_diag_write_reg(char dtype, unsigned char addr, char *write_data, unsigned char size, int flags)
{
    ...
    if (shdisp_panel_API_mipi_dsi_cmds_tx(1, cmd, ARRAY_SIZE(cmd), flags) != SHDISP_RESULT_SUCCESS){
...
/* ------------------------------------------------------------------------- */
int shdisp_panel_API_mipi_diag_read_reg(char dtype, unsigned char addr, unsigned char *read_data, unsigned char size, int flags)
{
#ifdef SHDISP_LOG_ENABLE
    int i;
    unsigned char rlen;
    ...
    if (shdisp_panel_API_mipi_dsi_cmds_rx(read_data, cmd, size, flags) != SHDISP_RESULT_SUCCESS){
...
/* ------------------------------------------------------------------------- */
int shdisp_panel_API_mipi_dsi_cmds_tx(int commit, struct shdisp_dsi_cmd_desc *cmds, int cnt, int flags)
{
    int ret;
    SHDISP_TRACE("in cnt=%d\n", cnt);
    shdisp_panel_dsi_wlog(cmds, cnt);
    ret = mdss_shdisp_host_dsi_tx(commit, cmds, cnt, flags);
...
/* ------------------------------------------------------------------------- */
int shdisp_panel_API_mipi_dsi_cmds_rx(unsigned char *rbuf, struct shdisp_dsi_cmd_desc *cmds, unsigned char size, int flags)
{
    int ret;
    SHDISP_TRACE("in size:%d\n", size);
    shdisp_panel_dsi_wlog(cmds, 1);
    ret = mdss_shdisp_host_dsi_rx(cmds, rbuf, size, flags);
...
/* ------------------------------------------------------------------------- */
/* shdisp_panel_API_digital_power_on                                         */
/* ------------------------------------------------------------------------- */

int shdisp_panel_API_digital_power_on(void)
{
    SHDISP_TRACE("in\n");
    if (shdisp_panel_fops->digital_power_on) {
        SHDISP_DEBUG("out1\n");
        return shdisp_panel_fops->digital_power_on();
    }
    SHDISP_TRACE("out\n");
    return SHDISP_RESULT_SUCCESS;
