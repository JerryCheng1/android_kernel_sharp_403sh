...
#if defined(CONFIG_MACH_LYNX_DL75)
#include "./data/shdisp_bl71y8_data_dl75.h"
#elif defined(CONFIG_MACH_PA28)
#include "./data/shdisp_bl71y8_data_pa28.h"
#else
#include "./data/shdisp_bl71y8_data_default.h"
#endif
...
static void shdisp_bdic_seq_led_emopattern_on(unsigned char color, int ontime, int interval, int count);
...
void shdisp_bdic_API_TRI_LED_emopattern_on(unsigned char color, int ontime, int interval, int count)
{
    shdisp_bdic_seq_led_emopattern_on(color, ontime, interval, count);
    return;
}
...
static void shdisp_bdic_seq_led_emopattern_on(unsigned char color, int ontime, int interval, int count)
{
    ...
    shdisp_bdic_seq_bdic_active_for_led(SHDISP_DEV_TYPE_LED);
    shdisp_bdic_PD_TRI_LED_control(SHDISP_BDIC_REQ_TRI_LED_SET_MODE_EMOPATTERN,   color);
    shdisp_bdic_PD_TRI_LED_control(SHDISP_BDIC_REQ_TRI_LED_SET_ONTIME,       ontime);
    shdisp_bdic_PD_TRI_LED_control(SHDISP_BDIC_REQ_TRI_LED_SET_INTERVAL,     interval);
    shdisp_bdic_PD_TRI_LED_control(SHDISP_BDIC_REQ_TRI_LED_SET_COUNT,        count);
    shdisp_bdic_PD_TRI_LED_control(SHDISP_BDIC_REQ_START, 0);
    ...
    if (ado_tmp != 0) {
        for (i = 0; i < SHDISP_BDIC_LUX_TABLE_ARRAY_SIZE; i++) {
            if ((ado_tmp >= shdisp_bdic_bkl_ado_tbl[i].range_low) &&
                ...
                ret_lux  = (unsigned long)(ado_tmp * (unsigned short)shdisp_bdic_bkl_ado_tbl[i].param_a);
...
{
    SHDISP_TRACE("in\n")
#if defined(CONFIG_MACH_LYNX_DL75)  ||defined(CONFIG_MACH_PA28)
...
static void shdisp_bdic_PD_TRI_LED_set_anime(void)
{
    ...
    case SHDISP_BDIC_TRI_LED_MODE_EMOPATTERN:
        ...
        shdisp_bdic_IO_write_reg(BDIC_REG_CH0_SET1, 0x03);
        shdisp_bdic_IO_write_reg(BDIC_REG_CH0_SET2, 0x05);
        shdisp_bdic_IO_write_reg(BDIC_REG_CH1_SET1, 0x05);
        shdisp_bdic_IO_write_reg(BDIC_REG_CH1_SET2, 0x05);
        shdisp_bdic_IO_write_reg(BDIC_REG_CH2_SET1, 0x03);
        shdisp_bdic_IO_write_reg(BDIC_REG_CH2_SET2, 0x05);
        shdisp_bdic_IO_msk_bit_reg(BDIC_REG_TIMEER, 0x00, 0xF0);
        break;
...
static void shdisp_bdic_PD_TRI_LED_set_chdig(void)
{
    ...
    case SHDISP_BDIC_TRI_LED_MODE_FIREFLY:
        ...
        break;
    case SHDISP_BDIC_TRI_LED_MODE_EMOPATTERN:
        wBuf[0] = shdisp_triple_led_emopattern_tbl[clrvari][0][SHDISP_TRI_LED_EMOPATTERN_COLOR-1][0];
        wBuf[1] = shdisp_triple_led_emopattern_tbl[clrvari][0][SHDISP_TRI_LED_EMOPATTERN_COLOR-1][1];
        wBuf[2] = shdisp_triple_led_emopattern_tbl[clrvari][0][SHDISP_TRI_LED_EMOPATTERN_COLOR-1][2];
        wBuf[3] = shdisp_triple_led_emopattern_tbl[clrvari][1][SHDISP_TRI_LED_EMOPATTERN_COLOR-1][0];
        wBuf[4] = shdisp_triple_led_emopattern_tbl[clrvari][1][SHDISP_TRI_LED_EMOPATTERN_COLOR-1][1];
        wBuf[5] = shdisp_triple_led_emopattern_tbl[clrvari][1][SHDISP_TRI_LED_EMOPATTERN_COLOR-1][2];
        wBuf[6] = shdisp_triple_led_emopattern_tbl[clrvari][2][SHDISP_TRI_LED_EMOPATTERN_COLOR-1][0];
        wBuf[7] = shdisp_triple_led_emopattern_tbl[clrvari][2][SHDISP_TRI_LED_EMOPATTERN_COLOR-1][1];
        wBuf[8] = shdisp_triple_led_emopattern_tbl[clrvari][2][SHDISP_TRI_LED_EMOPATTERN_COLOR-1][2];
        shdisp_bdic_IO_multi_write_reg(BDIC_REG_CH0_A, wBuf, 9);
        break;
    default:
        break;
...
static int shdisp_bdic_PD_psals_als_init_ps_off(void)
{
    ...
    ret = SHDISP_BDIC_REGSET(shdisp_bdic_set_bank0);
    if (ret != SHDISP_RESULT_SUCCESS) {
        SHDISP_ERR("out1\n");
    }
...
static int shdisp_bdic_PD_REG_ADO_get_opt(unsigned short *ado, unsigned short *clear, unsigned short *ir)
{
    ...
    range0 = rval[0] & 0x07;
    res    = (rval[0] & 0x38) >> 3;
    SHDISP_DEBUG("range0=%d, res=%d\n", range0, res);

    if(res != 0x05){
        for (retry=0;retry <SHDISP_BDIC_GET_ADO_RETRY_TIMES;retry++){
            ret = shdisp_photo_sensor_IO_read_reg(SENSOR_REG_COMMAND1, rval);
            if (ret != SHDISP_RESULT_SUCCESS) {
                SHDISP_ERR("out2");
                goto i2c_err;
            }
...
static void shdisp_bdic_PD_REG_RAW_DATA_get_opt(unsigned short *clear, unsigned short *ir)
{
    ...
    unsigned short als0, als1;
    unsigned char rval[(SENSOR_REG_D1_MSB+1)-SENSOR_REG_D0_LSB];
    unsigned char flag_a,range0,res;
