...
void shtps_ioctl_setlcdbrightlpmode_proc(struct shtps_rmi_spi *ts, u8 on)
{
	...
	shtps_set_lpmode(ts, SHTPS_LPMODE_TYPE_NON_CONTINUOUS, SHTPS_LPMODE_REQ_LCD_BRIGHT, (int)on);
		...
		if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) == 0 ){
			shtps_set_fw_report_rate(ts, SHTPS_CTRL_FW_REPORT_RATE_PARAM_DEFAULT);
		}else{
			shtps_set_fw_report_rate(ts, SHTPS_CTRL_FW_REPORT_RATE_PARAM);
		}
...
/* -----------------------------------------------------------------------------------
 */
#if defined(SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE)
static void shtps_set_baseline_checker(struct shtps_rmi_spi *ts, u8 val, u8 force)
{
	int rc = 0;
	u8 touch_detect[2] = {0};
	int face_object_detect = 0;

	if(SHTPS_PRM_CTRL_FW_BASELINE_CHECKER_ENABLE == 0){
		return;
	}

	SHTPS_LOG_FUNC_CALL_INPARAM(val);

	shtps_fwctl_get_ObjectAttention(ts, touch_detect);
	face_object_detect = shtps_fwctl_get_faceobject(ts);

	if(force != 0){
		if(val == SHTPS_ENM_SET_BASELINE_CHECKER_ENABLE){
			shtps_fwctl_set_baseline_checker(ts, 0);
		}else if(val == SHTPS_ENM_SET_BASELINE_CHECKER_DISABLE){
			shtps_fwctl_set_baseline_checker(ts, 1);
		}
	}else{
		if( (touch_detect[0] == 0) && (touch_detect[1] == 0) && (face_object_detect == 0) ){
			if(val == SHTPS_ENM_SET_BASELINE_CHECKER_ENABLE){
				rc = shtps_fwctl_set_baseline_checker(ts, 0);
			}else if(val == SHTPS_ENM_SET_BASELINE_CHECKER_DISABLE){
				rc = shtps_fwctl_set_baseline_checker(ts, 1);
			}
		}else{
			rc = 1;
		}
	...
	if(rc == 0){
		ts->baseline_checker_hold_set = SHTPS_ENM_HOLD_SET_BASELINE_CHECKER_NONE;
		SHTPS_LOG_DBG_PRINT("%s() set completed\n", __func__);
	}else{
		if(val == SHTPS_ENM_SET_BASELINE_CHECKER_ENABLE){
			ts->baseline_checker_hold_set = SHTPS_ENM_HOLD_SET_BASELINE_CHECKER_ENABLE;
		}else{
			ts->baseline_checker_hold_set = SHTPS_ENM_HOLD_SET_BASELINE_CHECKER_DISABLE;
		}
		SHTPS_LOG_DBG_PRINT("%s() set fail <hold = %s>\n", __func__,
								(ts->baseline_checker_hold_set == SHTPS_ENM_HOLD_SET_BASELINE_CHECKER_ENABLE) ? "enable" : "disable");
	}
}

static void shtps_set_baseline_checker_hold_check(struct shtps_rmi_spi *ts, u8 force)
{
	if(SHTPS_PRM_CTRL_FW_BASELINE_CHECKER_ENABLE == 0){
		return;
	}
	...
	if(ts->baseline_checker_hold_set == SHTPS_ENM_HOLD_SET_BASELINE_CHECKER_NONE){
		return;
	}

	if(ts->baseline_checker_hold_set == SHTPS_ENM_HOLD_SET_BASELINE_CHECKER_ENABLE){
		shtps_set_baseline_checker(ts, SHTPS_ENM_SET_BASELINE_CHECKER_ENABLE, force);
	}else if(ts->baseline_checker_hold_set == SHTPS_ENM_HOLD_SET_BASELINE_CHECKER_DISABLE){
		shtps_set_baseline_checker(ts, SHTPS_ENM_SET_BASELINE_CHECKER_DISABLE, force);
	}
}
#endif /* SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE */
...
static void shtps_set_facetouchmode(struct shtps_rmi_spi *ts, int mode)
{
	...
	if(mode == 0){
		...
		#if defined(SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE)
			shtps_set_baseline_checker(ts, SHTPS_ENM_SET_BASELINE_CHECKER_ENABLE, 0);
		#endif /* SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE */

		#if defined( SHTPS_FACETOUCH_OFF_DETECT_SKIP_REZERO_ENABLE )
			ts->facetouch.call_start_rezero_disable_check_state = 0;
		#endif /* SHTPS_FACETOUCH_OFF_DETECT_SKIP_REZERO_ENABLE */
	}else{
		#if defined(SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE)
			shtps_set_baseline_checker(ts, SHTPS_ENM_SET_BASELINE_CHECKER_DISABLE, 0);
		#endif /* SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE */

		#if defined( SHTPS_FACETOUCH_OFF_DETECT_SKIP_REZERO_ENABLE )
			ts->facetouch.call_start_rezero_disable_check_state = 1;
			do_gettimeofday(&ts->facetouch.call_start_time);
		#endif /* SHTPS_FACETOUCH_OFF_DETECT_SKIP_REZERO_ENABLE */
	}
...
//	if(SHTPS_FACETOUCH_DETECT_CHECK_PALMFLAG == 1 && (info->gs1 & 0x02) == 0x02){
	if(SHTPS_FACETOUCH_DETECT_CHECK_PALMFLAG == 1 && shtps_chck_palm(ts) == 1){
		...
		palm_det = 1;
	}else if(SHTPS_FACETOUCH_DETECT_CHECK_FACE_OBJECT_EXISTS != 0 && ts->faceobject != 0){
		SHTPS_LOG_DBG_PRINT("Detect palm touch by face object flag.\n");
		palm_det = 1;
	}else{
		for(i = 0;i < fingerMax;i++){
...
static void shtps_read_touchevent_infacetouchmode(struct shtps_rmi_spi *ts)
{
	...
	ts->faceobject = 0;
	#if defined(SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE)
		shtps_set_baseline_checker_hold_check(ts, 0);
	#endif /* SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE */
...
void shtps_set_fw_report_rate(struct shtps_rmi_spi *ts, u8 rate)
{
	int ret;
		...
		if( ((ts->lpmode_req_state | ts->lpmode_continuous_req_state) & SHTPS_LPMODE_REQ_LCD_BRIGHT) == 0 ){
			...
			ret = shtps_fwctl_set_custom_report_rate(ts, rate);
		}
	#else
		ret = shtps_fwctl_set_custom_report_rate(ts, rate);
	#endif /* SHTPS_DEF_CTRL_FW_REPORT_RATE_LINKED_LCD_BRIGHT_ENABLE */
...
static void shtps_get_multi_fw_type(struct shtps_rmi_spi *ts)
{
	...
	{
		u8 i;
		u8 hwrev = SHTPS_GET_HW_VERSION_RET_PP_2;
...
static int shtps_init_param(struct shtps_rmi_spi *ts)
{
	...
	#if defined(SHTPS_CTRL_FW_REPORT_RATE)
		shtps_set_fw_report_rate_init(ts);
	#endif /* SHTPS_CTRL_FW_REPORT_RATE */
...
static void shtps_set_touch_info(struct shtps_rmi_spi *ts, u8 *buf, struct shtps_touch_info *info)
{
	...
	for(i=0;i<fingerMax;i++){
		...
		if( (ts->state_mgr.state == SHTPS_STATE_FACETOUCH) || (ts->state_mgr.state == SHTPS_STATE_SLEEP_FACETOUCH) ){
			SHTPS_LOG_EVENT(
				DBG_PRINTK("Face Object Exists[%d]\n", ts->faceobject);
			);
		}
...
void shtps_read_touchevent(struct shtps_rmi_spi *ts, int state)
{
	...
	#if defined( CONFIG_SHTPS_SY3000_FACETOUCH_DETECT ) || defined( CONFIG_SHTPS_SY3000_FACETOUCH_OFF_DETECT )
		if( (ts->state_mgr.state == SHTPS_STATE_FACETOUCH) || (ts->state_mgr.state == SHTPS_STATE_SLEEP_FACETOUCH) ){
			ts->faceobject = shtps_fwctl_get_faceobject(ts);
		}else{
			ts->faceobject = 0;
		}
	#endif /* CONFIG_SHTPS_SY3000_FACETOUCH_DETECT || CONFIG_SHTPS_SY3000_FACETOUCH_OFF_DETECT */
...
static int shtps_statef_sleep_enter(struct shtps_rmi_spi *ts, int param)
{
	...
	#if defined(SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE)
		shtps_set_baseline_checker_hold_check(ts, 1);
	#endif /* SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE */
...
static int shtps_statef_sleep_facetouch_wakeup(struct shtps_rmi_spi *ts, int param)
{
...
#if defined( SHTPS_FACETOUCH_OFF_DETECT_SKIP_REZERO_ENABLE )
	{
		...
		long now_diff_time_s = 0;
		long now_diff_time_us = 0;
		u8 is_skip_rezero = 0;

		if(SHTPS_FACETOUCHOFF_DETECT_SKIP_REZERO_ENABLE != 0){
			if(ts->facetouch.call_start_rezero_disable_check_state != 0){
				do_gettimeofday(&now_tv);
				now_diff_time_s = now_tv.tv_sec - ts->facetouch.call_start_time.tv_sec;
				now_diff_time_us = (now_tv.tv_sec * 1000000 + now_tv.tv_usec) -
									(ts->facetouch.call_start_time.tv_sec * 1000000 +
										ts->facetouch.call_start_time.tv_usec);

				SHTPS_LOG_DBG_PRINT("rezero skip time check diff_time = <%lds, %ldus>\n", now_diff_time_s, now_diff_time_us);

				if(SHTPS_FACETOUCHOFF_DETECT_SKIP_REZERO_FIRST_TIME_MS > 0){
					if( (now_diff_time_s <= (SHTPS_FACETOUCHOFF_DETECT_SKIP_REZERO_FIRST_TIME_MS / 1000)) &&
						(now_diff_time_us <= (SHTPS_FACETOUCHOFF_DETECT_SKIP_REZERO_FIRST_TIME_MS * 1000)) )
					{
						is_skip_rezero = 1;
						SHTPS_LOG_DBG_PRINT("Detect call start req rezero. skip rezero...\n");
					}
				}
				ts->facetouch.call_start_rezero_disable_check_state = 0;
			}

			if(ts->facetouch.rezero_disable == 1){
				ts->facetouch.rezero_disable = 0;
				is_skip_rezero = 1;
