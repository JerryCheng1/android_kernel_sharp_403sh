...
static int shtps_fwctl_s3400_get_lpwg_def_settings(struct shtps_fwctl_info *fc_p)
{
				...
				fc_p->map_p->fn12_ctrl8_disable_settings, sizeof(fc_p->map_p->fn12_ctrl8_enable_settings));
	{
		...
		if(SHTPS_LPWG_RECEIVER_PITCH != 0x0000){
			fc_p->map_p->fn12_ctrl8_enable_settings[4] = (SHTPS_LPWG_RECEIVER_PITCH & 0xFF);
			fc_p->map_p->fn12_ctrl8_enable_settings[5] = ((SHTPS_LPWG_RECEIVER_PITCH >> 8) & 0xFF);
		}
		if(SHTPS_LPWG_TRANSMITTER_PITCH != 0x0000){
			fc_p->map_p->fn12_ctrl8_enable_settings[6] = (SHTPS_LPWG_TRANSMITTER_PITCH & 0xFF);
			fc_p->map_p->fn12_ctrl8_enable_settings[7] = ((SHTPS_LPWG_TRANSMITTER_PITCH >> 8) & 0xFF);
		}
...
/* -------------------------------------------------------------------------- */
static int shtps_fwctl_s3400_get_faceobject(struct shtps_fwctl_info *fc_p)
{
	u8 val = 0;

	#if defined( CONFIG_SHTPS_SY3000_FACETOUCH_DETECT )
		M_READ_FUNC(fc_p, fc_p->map_p->fn51.dataBase + F51_QUERY_FACEDETECTIONDATAREGISTERADDRESS(fc_p->map_p->fn51.query.data), &val, 1);
	#endif /* CONFIG_SHTPS_SY3000_FACETOUCH_DETECT */

	return val;
}

/* -------------------------------------------------------------------------- */
static int shtps_fwctl_s3400_set_baseline_checker(struct shtps_fwctl_info *fc_p, u8 disable)
{
	int rc = 0;
	...
	#if defined(SHTPS_CTRL_FW_BASELINE_CHECKER_ENABLE)
	{
		...
		if(disable == 0){
			rc = M_WRITE_FUNC(fc_p, fc_p->map_p->fn51.dataBase + F51_QUERY_BASELINECHECKERREGISTERADDRESS(fc_p->map_p->fn51.query.data), 0x00);
		}else{
			rc = M_WRITE_FUNC(fc_p, fc_p->map_p->fn51.dataBase + F51_QUERY_BASELINECHECKERREGISTERADDRESS(fc_p->map_p->fn51.query.data), 0x01);
		}
...
struct shtps_fwctl_functbl		shtps_fwctl_s3400_function_table = {
		...
		.get_faceobject_f							= shtps_fwctl_s3400_get_faceobject,
		.set_baseline_checker_f						= shtps_fwctl_s3400_set_baseline_checker,
};
/* -------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------- */
/* -------------------------------------------------------------------------- */
