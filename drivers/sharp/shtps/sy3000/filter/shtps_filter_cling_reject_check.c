...
struct shtps_filter_cling_reject_info{
	...
	u8								mode5_cling_cnt_y[SHTPS_FINGER_MAX];
	u8								mode5_cling_cnt_x[SHTPS_FINGER_MAX];
	u8								mode5_cling_tu_cnt[SHTPS_FINGER_MAX];
	u8								mode6_cling_status;
	u16								mode6_cling_cnt[SHTPS_FINGER_MAX];
};
...
static u8 shtps_filter_cling_reject_check_finger(struct shtps_rmi_spi *ts)
{
	...
	if( SHTPS_CLING_REJECT_FINGER_MODE_5_ENABLE != 0 )
	{
		int numOfFingers = 0;
		...
		for(i = 0; i < fingerMax; i++){
			...
			{
					if( ( ts->fw_report_info.fingers[i].wx == ts->fw_report_info_store.fingers[i].wx ) &&
					    ( ts->fw_report_info.fingers[i].wy == ts->fw_report_info_store.fingers[i].wy ) )
					{
						cling_reject_p->mode5_cling_cnt_x[i]++;
						cling_reject_p->mode5_cling_cnt_y[i]++;
					}
					else if( ts->fw_report_info.fingers[i].wy == ts->fw_report_info_store.fingers[i].wy )
					{
						cling_reject_p->mode5_cling_cnt_y[i]++;
						cling_reject_p->mode5_cling_cnt_x[i] = 0;
					}
					else if( ts->fw_report_info.fingers[i].wx == ts->fw_report_info_store.fingers[i].wx )
					{
						cling_reject_p->mode5_cling_cnt_y[i] = 0;
						cling_reject_p->mode5_cling_cnt_x[i]++;
					}
					else{
						cling_reject_p->mode5_cling_cnt_y[i] = 0;
					...
					SHTPS_LOG_CLING_REJECT("[mode_5] ID=[%d] cntup x[%d] cntup y[%d] cnt tu[%d] \n",i,cling_reject_p->mode5_cling_cnt_x[i],cling_reject_p->mode5_cling_cnt_y[i],cling_reject_p->mode5_cling_tu_cnt[i]);
			}
					 ...
					 ( ts->fw_report_info_store.fingers[i].state == SHTPS_TOUCH_STATE_FINGER )) 
	 		{
				if( numOfFingers == 0 )
				{
					cling_reject_p->mode5_cling_tu_cnt[i] = 0;
				}else{
					cling_reject_p->mode5_cling_tu_cnt[i]++;
				}
			}
			else{
				cling_reject_p->mode5_cling_cnt_y[i] = 0;
			...
			if( ( cling_reject_p->mode5_cling_cnt_x[i] >= SHTPS_CLING_REJECT_FINGER_MODE_5_CNT_MAX ) ||
			    (cling_reject_p->mode5_cling_cnt_y[i] >= SHTPS_CLING_REJECT_FINGER_MODE_5_CNT_MAX  ) )
			{
				for(j = 0; j < fingerMax; j++){
					if( cling_reject_p->mode5_cling_tu_cnt[j] >= SHTPS_CLING_REJECT_FINGER_MODE_5_TU_CNT_MAX )
					{
						SHTPS_LOG_CLING_REJECT("[mode_5] ID =[%d] cntover x[%d] y[%d] tu[%d] \n",i,cling_reject_p->mode5_cling_cnt_x[i],cling_reject_p->mode5_cling_cnt_y[i],	cling_reject_p->mode5_cling_tu_cnt[i]);
						is_cling_detect = 1;
					}
					else{
						cling_reject_p->mode5_cling_tu_cnt[j] = 0;
					}
				}
				cling_reject_p->mode5_cling_cnt_y[i] = 0;
				cling_reject_p->mode5_cling_cnt_x[i] = 0;
	...
	if( SHTPS_CLING_REJECT_FINGER_MODE_6_ENABLE != 0 )
	{
		int numOfFingers = 0;
		...
		cling_reject_p->mode6_cling_status = 0;
		for(i = 0; i < fingerMax; i++){
			if( ( ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_FINGER      ) &&
			    ( ts->fw_report_info_store.fingers[i].state == SHTPS_TOUCH_STATE_FINGER)){

				if (( ( ts->fw_report_info.fingers[i].x >= CONFIG_SHTPS_SY3000_LCD_SIZE_X - SHTPS_CLING_REJECT_FINGER_MODE_6_THRESH) ||
				     ( ts->fw_report_info.fingers[i].y >= CONFIG_SHTPS_SY3000_LCD_SIZE_Y - SHTPS_CLING_REJECT_FINGER_MODE_6_THRESH) )  ||

			     (( ( ts->fw_report_info.fingers[i].x <= SHTPS_CLING_REJECT_FINGER_MODE_6_THRESH ) ||
					     ( ts->fw_report_info.fingers[i].y <= SHTPS_CLING_REJECT_FINGER_MODE_6_THRESH ) ) && 
					   ( numOfFingers >= SHTPS_CLING_REJECT_FINGER_MODE_6_FINGER_MAX )))
				{
					cling_reject_p->mode6_cling_status = 1;
				}
			}
			else{
				cling_reject_p->mode6_cling_cnt[i] = 0;
			}
		}
		...
		for(i = 0; i < fingerMax; i++){
			if( ( ts->fw_report_info.fingers[i].state == SHTPS_TOUCH_STATE_FINGER      ) &&
				...
				if( ( ts->fw_report_info.fingers[i].wx == ts->fw_report_info_store.fingers[i].wx ) &&
				    ...
				    ( cling_reject_p->mode6_cling_status == 1))
				{
					cling_reject_p->mode6_cling_cnt[i]++;
				}
				else{
					cling_reject_p->mode6_cling_cnt[i] = 0;
				}

				SHTPS_LOG_CLING_REJECT("[mode_6] status=[%d] ID =[%d] cnt [%d]\n",cling_reject_p->mode6_cling_status,i,cling_reject_p->mode6_cling_cnt[i] );

				if( cling_reject_p->mode6_cling_cnt[i] >= SHTPS_CLING_REJECT_FINGER_MODE_6_CNT_MAX )
				{
					SHTPS_LOG_CLING_REJECT("[mode_6] ID =[%d] cntover[%d]\n",i,cling_reject_p->mode6_cling_cnt[i]);
					is_cling_detect = 1;
				}
			}
		}
...
void shtps_filter_cling_reject_check(struct shtps_rmi_spi *ts, struct shtps_touch_info *info)
{
	...
	if(ts->state_mgr.state != SHTPS_STATE_ACTIVE){
		SHTPS_LOG_CLING_REJECT("%s() do not process by drv state[%d]\n", __func__, ts->state_mgr.state);
		return;
	}
