...
{
	return ts_p->fwctl_p->fwctl_func_p->get_faceobject_f(ts_p->fwctl_p);
}

/* -------------------------------------------------------------------------- */
int shtps_fwctl_set_baseline_checker(struct shtps_rmi_spi *ts_p, u8 disable)
{
	return ts_p->fwctl_p->fwctl_func_p->set_baseline_checker_f(ts_p->fwctl_p, disable);
}
