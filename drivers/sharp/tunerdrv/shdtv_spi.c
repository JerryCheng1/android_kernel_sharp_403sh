...
static int __devinit shdtv_dev_spi_probe(struct spi_device *spi)
{
	...
	ret = shdtvspi_sem_init();
	if (ret) {
		printk("%s:%d shdtvspi_sem_init() failed \n", __FILE__, __LINE__);
		return ret;
	}
...
{
	SHDTV_DBG("shdtvspi_sem_lock() start  pid = %ld \n", sys_getpid());
	...
	SHDTV_DBG("shdtvspi_sem_lock() end  pid = %ld \n", sys_getpid());
	return 0;
}

int shdtvspi_sem_unlock(void)
{
	SHDTV_DBG("shdtvspi_sem_unlock() start  pid = %ld \n", sys_getpid());
	...
	SHDTV_DBG("shdtvspi_sem_unlock() end  pid = %ld \n", sys_getpid());
	return 0;
}
