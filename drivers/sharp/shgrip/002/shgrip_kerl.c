...
#if defined(CONFIG_DTV_TUNERDEV)
#include <sharp/shdtv_spi_ext.h>
#include <linux/semaphore.h>
#endif /* CONFIG_DTV_TUNERDEV */
...
#define SHGRIP_SPI_MOSI						(8)
#define SHGRIP_SPI_MISO						(9)
#define SHGRIP_SPI_CS_N						(110)
#define SHGRIP_SPI_CLK						(11)

#define SHGRIP_GPIO_GRIP_INT				(67)

#define SHGRIP_GPIO_GRIP_RESET				(91)
...
#define SHGRIP_CMD_CH0EPCC					(0x404B)
#define SHGRIP_CMD_CH4ICS					(0x406A)
#define SHGRIP_CMD_CH4EPCC					(0x406B)
...
module_param(shgrip_dtv_info, int, 0600);
#endif /* CONFIG_ANDROID_ENGINEERING */
...
#define SHGRIP_DTV_INFO(fmt, args...) \
		if(shgrip_dtv_info == 1) { \
			printk("[SHGRIP_DTV_INFO][%s] " fmt, __func__, ## args); \
		}
...
/* shgrip_dtv_sem_lock                                                       */
/* ------------------------------------------------------------------------- */
static int shgrip_dtv_sem_lock(void)
{
	int ret = 0;
...
/* ------------------------------------------------------------------------- */
/* shgrip_dtv_sem_unlock                                                     */
/* ------------------------------------------------------------------------- */
static void shgrip_dtv_sem_unlock(void)
{
#if defined(CONFIG_DTV_TUNERDEV)
...
static int shgrip_cmd_gripsr(struct shgrip_sensor_state *ch_state)
{
	...
	SHGRIP_DTV_INFO("shgrip_dtv_sem_lock() start\n");
	ret = shgrip_dtv_sem_lock();
	if (ret) {
		SHGRIP_ERR("shgrip_dtv_sem_lock() failed. ret:%d\n", ret);
		return ret;
	}
	...
	if (ret) {
		...
		shgrip_dtv_sem_unlock();
		SHGRIP_DTV_INFO("shgrip_dtv_sem_unlock() end\n");
		return ret;
	}
	shgrip_dtv_sem_unlock();
	SHGRIP_DTV_INFO("shgrip_dtv_sem_unlock() end\n");
...
static void shgrip_th_dump_func(struct work_struct *work)
{
	...
	unsigned char ch0ics_data = 0;
	unsigned char ch0epcc_data = 0;
	unsigned char ch4ics_data = 0;
	unsigned char ch4epcc_data = 0;
	...
	case SHGRIP_STATE_SENSOR_ON:
		...
		ret = shgrip_spi_transfer_read_block(SHGRIP_CMD_CH0ICS, &ch0ics_data, 1);
		if (ret) {
			SHGRIP_ERR("ch0ics_data read err. ret:%d\n", ret);
		}

		ret = shgrip_spi_transfer_read_block(SHGRIP_CMD_CH0EPCC, &ch0epcc_data, 1);
		if (ret) {
			SHGRIP_ERR("ch0epcc_data read err. ret:%d\n", ret);
		}

		ret = shgrip_spi_transfer_read_block(SHGRIP_CMD_CH4ICS, &ch4ics_data, 1);
		if (ret) {
			SHGRIP_ERR("ch4ics_data read err. ret:%d\n", ret);
		}
		
		ret = shgrip_spi_transfer_read_block(SHGRIP_CMD_CH4EPCC, &ch4epcc_data, 1);
		if (ret) {
			SHGRIP_ERR("ch4epcc_data read err. ret:%d\n", ret);
		}
		shgrip_dtv_sem_unlock();
		SHGRIP_DTV_INFO("shgrip_dtv_sem_unlock() end\n");
		...
		SHGRIP_INFO("CH0 Dlt:0x0000, Ref:0x%02X%02X, Meas:0x%02X%02X, PROXTH:0x%02X%02X, TouchTH:0x%02X%02X, ReleaseTH:0x%02X, GRIPSR:0x%02X, CHSR:0x%02X, CH0ICS:0x%02X, CH0EPCC:0x%02X\n",
			...
			ch0th_buf[0], ch0th_buf[1], ch0th_buf[2], ch0th_buf[3], ch0th_buf[4], gripsr_data, ch0sr_data, ch0ics_data, ch0epcc_data );
		SHGRIP_INFO("CH4 Dlt:0x0000, Ref:0x%02X%02X, Meas:0x%02X%02X, PROXTH:0x%02X%02X, TouchTH:0x%02X%02X, ReleaseTH:0x%02X, GRIPSR:0x%02X, CHSR:0x%02X, CH4ICS:0x%02X, CH4EPCC:0x%02X\n",
			...
			ch4th_buf[0], ch4th_buf[1], ch4th_buf[2], ch4th_buf[3], ch4th_buf[4], gripsr_data, ch4sr_data, ch4ics_data, ch4epcc_data );
...
static int shgrip_dev_init(void)
{
	...
	gpio_request(SHGRIP_GPIO_GRIP_RESET, NULL);
	ret = gpio_tlmm_config(GPIO_CFG(SHGRIP_GPIO_GRIP_RESET, 0,
			GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (ret) {
		SHGRIP_ERR("GPIO(RESET) set config NG\n");
		goto shgrip_gpio_rst_err;
	}
	ret = gpio_direction_output(SHGRIP_GPIO_GRIP_RESET, 1);
	if (ret) {
		SHGRIP_ERR("GPIO(RESET) set direction NG\n");
		goto shgrip_gpio_rst_err;
	}
...
shgrip_spi_register_err:
	shgrip_sys_free_irq();
shgrip_gpio_rst_err:
	gpio_free(SHGRIP_GPIO_GRIP_RESET);
shgrip_gpio_int_err:
