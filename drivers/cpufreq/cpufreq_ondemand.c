...
static void update_sampling_rate(unsigned int new_rate)
{
	...
	if (sampling_rate_boosted && new_rate > DEFAULT_SAMPLING_RATE) {
	...
	if (sampling_rate_boosted) {
		current_sampling_rate = max(new_rate, min_sampling_rate);
		return;
	} else {
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	if (!dbs_tuners_ins.powersave_bias ||
		...
		if (dbs_info->freq_lo) {
			...
			if (sh_cpufreq_adjust.mdp_clk_ena || sh_cpufreq_adjust.kgsl_is_working || sh_cpufreq_adjust.music_type) {
				delay = usecs_to_jiffies(SAMPLING_LATE_MDSS_MDP_CLK_ENABLE * dbs_info->rate_mult);
				pr_debug("cpu%d : sampling rate -> %d us, mdp_clk = %d gpu_clk = %d is_music_play = %d check clock up elapsed time \n", cpu, (int)(SAMPLING_LATE_MDSS_MDP_CLK_ENABLE*dbs_info->rate_mult),  sh_cpufreq_adjust.mdp_clk_ena, sh_cpufreq_adjust.kgsl_is_working, sh_cpufreq_adjust.music_type);
			}else{
