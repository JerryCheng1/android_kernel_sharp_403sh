...
/*                                                                         */
/* ----------------------------------------------------------------------- */
static void mdss_shdisp_kick_collect_cmd(int flags)
{
	int iscmdmodeon;
	...
	cmdreq.flags = CMD_REQ_COMMIT | flags;
...
/*                                                                         */
/* ----------------------------------------------------------------------- */
int mdss_shdisp_host_dsi_tx(int commit, struct shdisp_dsi_cmd_desc * shdisp_cmds, int size, int flags)
{
	if (!mdss_dsi_ctrl) {
		pr_err("LCDERR: %s mdss_dsi_ctrl=0x%p", __func__, mdss_dsi_ctrl);
	...
	if (commit) {
		mdss_shdisp_kick_collect_cmd(flags);
	}
...
/*                                                                         */
/* ----------------------------------------------------------------------- */
int mdss_shdisp_host_dsi_rx(struct shdisp_dsi_cmd_desc * cmds, unsigned char * rx_data, int rx_size, int flags)
{
	struct dcs_cmd_req cmdreq;
	...
	cmdreq.flags = CMD_REQ_RX | CMD_REQ_COMMIT | flags;
...
void mdss_shdisp_digital_power_on(void)
{
	shdisp_api_digital_power_on();
}
